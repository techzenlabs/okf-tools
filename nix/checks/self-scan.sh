...
# is the gate rather than the habit. The planted fixtures are excluded by
# name, and every run prints its exclusions, so an exemption is never silent.
set -euo pipefail
export HOME="$TMPDIR"
tree="$TMPDIR/tree"
cp -r "$SOURCE" "$tree"
...
okf-scan --exclude fixtures/scan-planted --exclude fixtures/site/alpha-planted \
  --exclude fixtures/site/planted-static .
